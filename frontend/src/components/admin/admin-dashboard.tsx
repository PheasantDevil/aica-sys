...
export function AdminDashboard({ className }: AdminDashboardProps) {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      // Simulate API calls
      const [statsData, logsData] = await Promise.all([fetchSystemStats(), fetchLogs()]);
  ...
  return (
    <div className={cn("space-y-6", className)}>
      ...
      <div className="space-y-6">
        ...
        {selectedTab === "logs" && (
          <LogsTab logs={filteredLogs} filter={logFilter} onFilterChange={setLogFilter} />
        )}
        ...
        <div className="bg-card p-6 rounded-lg border">
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium text-red-600">{stats.performance.errorRate}%</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-card p-6 rounded-lg border">
          ...
          <div className="space-y-4">
            <div className="flex justify-between">
              ...
              <span className="font-medium">${stats.revenue.monthly.toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Yearly</span>
              <span className="font-medium">${stats.revenue.yearly.toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Growth</span>
              <span className="font-medium text-green-600">+{stats.revenue.growth}%</span>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-4">
        <select
          value={filter}
          onChange={(e) => onFilterChange(e.target.value as "all" | "error" | "warning" | "info")}
