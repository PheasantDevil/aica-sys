...
import { Notification, NotificationManager, useNotifications } from "@/lib/notifications";
...
function NotificationItem({ notification, onClick, onAction }: NotificationItemProps) {
  const t = useTranslations("common");
            ...
            <div className="mt-3 flex gap-2">
              {notification.actions.map((action) => (
                <AccessibleButton
                  ...
                  size="sm"
                  variant={
                    (action.variant === "primary" ? "default" : action.variant) || "secondary"
                  }
                  onClick={(e) => {
                    e.stopPropagation();
                    onAction(action.id);
          ...
          {Object.entries(preferences.categories).map(([category, enabled]) => (
            <label key={category} className="flex items-center gap-3">
              <input
                type="checkbox"
                checked={enabled}
                onChange={(e) => handlePreferenceChange(`categories.${category}`, e.target.checked)}
                className="rounded border-input"
              />
