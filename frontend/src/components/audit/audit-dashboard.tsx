...
import { AlertTriangle, CheckCircle, Download, Filter, Info, Search, Shield } from "lucide-react";
...
const getEventTypeBadge = (eventType: string) => {
  switch (eventType) {
    case "USER_LOGIN":
    case "USER_LOGOUT":
      return <Badge className="bg-blue-500 text-white">Auth</Badge>;
    case "USER_REGISTRATION":
      return <Badge className="bg-green-500 text-white">Registration</Badge>;
    case "DATA_MODIFICATION":
      return <Badge className="bg-yellow-500 text-white">Modification</Badge>;
    case "DATA_DELETION":
...
const AuditDashboard = () => {
  ...
  const {
    ...
    queryKey: ["auditDashboard", dateRange?.from?.toISOString(), dateRange?.to?.toISOString()],
    queryFn: () =>
      fetchAuditDashboard(dateRange?.from?.toISOString(), dateRange?.to?.toISOString()),
  ...
  if (dashboardError)
    return <div className="text-center py-8 text-red-500">Error: {dashboardError.message}</div>;
  if (eventsError)
    return (
  ...
  if (!dashboardData) return <div className="text-center py-8">No audit data available.</div>;
    ...
    <div className="container mx-auto p-6 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <Card className="shadow-lg">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{dashboardData.stats.total_events}</div>
        ...
        <Card className="shadow-lg">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{dashboardData.stats.success_rate.toFixed(1)}%</div>
            <p className="text-xs text-gray-500">
        ...
        <Card className="shadow-lg">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{dashboardData.stats.top_users.length}</div>
            <p className="text-xs text-gray-500">
        ...
        <Card className="shadow-lg">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{dashboardData.stats.top_resources.length}</div>
            <p className="text-xs text-gray-500">
              Most accessed: {dashboardData.stats.top_resources[0]?.resource_type || "N/A"}
            </p>
          </CardContent>
        </Card>
      </div>
              ...
              auditEvents.map((event, index) => (
                <div key={event.id} className="mb-4 last:mb-0">
                  ...
                  <div className="text-sm text-gray-700 ml-2">
                    ...
                    <p>
                      Resource: {event.resource_type}/{event.resource_id || "N/A"}
                    </p>
                    <p>IP: {event.ip_address || "N/A"}</p>
                    {event.event_data && (
                      <p>Data: {JSON.stringify(event.event_data).substring(0, 100)}...</p>
                    )}
                  </div>
                  {index < auditEvents.length - 1 && <Separator className="my-2" />}
                </div>
              ))
            ) : (
