  ...
  const handleShare = (platform: string) => {
    ...
    switch (platform) {
      ...
      case 'copy':
        ...
        analyticsEvents.trackEvent('copy_link', 'engagement', 'social_share');
        return;
    }

    if (shareUrl) {
      window.open(shareUrl, '_blank', 'width=600,height=400');
      analyticsEvents.trackEvent('social_share', 'engagement', platform);
    }
  };
  ...
  const handleNativeShare = async () => {
    if (navigator.share) {
      try {
        ...
        analyticsEvents.trackEvent('native_share', 'engagement', 'web_share_api');
      } catch (error) {
