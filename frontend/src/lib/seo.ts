...
export class SEOUtils {
  ...
  private static defaultDescription =
    "AI駆動型TypeScriptエコシステム特化型の自動コンテンツ生成・販売システム";
    ...
    switch (type) {
      ...
      case "organization":
        return {
          ...
          sameAs: ["https://twitter.com/aica_sys", "https://github.com/PheasantDevil/aica-sys"],
        };
...
export function useSEO() {
  ...
  const optimizeTitle = (title: string, maxLength?: number) =>
    SEOUtils.optimizeTitle(title, maxLength);
  const optimizeDescription = (description: string, maxLength?: number) =>
    SEOUtils.optimizeDescription(description, maxLength);
  const extractKeywords = (text: string, maxKeywords?: number) =>
    SEOUtils.extractKeywords(text, maxKeywords);
  ...
  const generateOGImageUrl = (title: string, description?: string) =>
    SEOUtils.generateOGImageUrl(title, description);
