...
async function getArticles(): Promise<
  Array<{ slug: string; createdAt: string; updatedAt?: string }>
> {
  try {
    const response = await fetch(
      `${
        process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"
      }/api/content/articles?limit=1000`,
      {
      ...
      data.articles?.map((article: any) => ({
        slug:
          article.title
            ?.toLowerCase()
            .replace(/\s+/g, "-")
            .replace(/[^a-z0-9-]/g, "") || `article-${article.id}`,
...
async function getNewsletters(): Promise<
  Array<{ slug: string; createdAt: string; updatedAt?: string }>
> {
  try {
    const response = await fetch(
      `${
        process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"
      }/api/content/newsletters?limit=1000`,
      {
        next: { revalidate: 3600 }, // 1時間キャッシュ
      },
    );
      ...
      data.newsletters?.map((newsletter: any) => ({
        slug:
          newsletter.title
            ?.toLowerCase()
            .replace(/\s+/g, "-")
            .replace(/[^a-z0-9-]/g, "") || `newsletter-${newsletter.id}`,
...
> {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000"}/api/content/trends?limit=1000`,
      {
        next: { revalidate: 3600 }, // 1時間キャッシュ
      },
    );
      ...
      data.trends?.map((trend: any) => ({
        slug:
          trend.title
            ?.toLowerCase()
            .replace(/\s+/g, "-")
            .replace(/[^a-z0-9-]/g, "") || `trend-${trend.id}`,
