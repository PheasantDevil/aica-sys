...
class RateLimiter:
    def __init__(self):
        self.default_limits = {
            ...
            "stripe_webhook": {"requests": 1000, "window": 60},  # 1000 webhooks per minute
        }
    ...
    def is_rate_limited(
        ...
        limits = custom_limits or self.default_limits.get(limit_type, {"requests": 100, "window": 3600})
...
def rate_limit(limit_type: str, custom_limits: Optional[Dict[str, int]] = None):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            # Check rate limit
            is_limited, info = rate_limiter.is_rate_limited(identifier, limit_type, custom_limits)
...
class SlidingWindowRateLimiter:
    ...
    def is_allowed(
        ...
        window_data = cache_manager.get(cache_key) or {"requests": [], "last_cleanup": current_time}
        ...
        if len(window_data["requests"]) >= requests_limit:
            return False, {
                ...
                "limit": requests_limit,
                "window": window_seconds,
                "oldest_request": min(window_data["requests"]) if window_data["requests"] else None
            }
