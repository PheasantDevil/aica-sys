...
class MonitoringMiddleware(BaseHTTPMiddleware):
    ...
    async def _record_request_metrics(self, request_info: Dict[str, Any], 
    ...
    async def _record_error_metrics(self, request_info: Dict[str, Any], 
                                  error_message: str, response_time: float):
        """エラーメトリクスを記録"""
        self.error_count += 1
    ...
    async def _send_metrics_to_monitoring_service(self, request_info: Dict[str, Any], 
                                                response_info: Dict[str, Any]):
        """監視サービスにメトリクスを送信"""
        try:
            ...
            if _monitoring_service_instance:
                _monitoring_service_instance.metrics.extend([response_time_metric, request_count_metric])
            
        except Exception as e:
            logger.error(f"Failed to send metrics to monitoring service: {e}")
    
    async def _send_error_metrics_to_monitoring_service(self, request_info: Dict[str, Any], 
                                                      error_message: str, response_time: float):
        """監視サービスにエラーメトリクスを送信"""
        try:
            from services.monitoring_service import Metric, MetricType, Alert, AlertLevel
            from datetime import datetime
