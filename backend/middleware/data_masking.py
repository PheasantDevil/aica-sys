...
from services.data_classification import (DataClassificationService,
...
class DataMaskingMiddleware(BaseHTTPMiddleware):
    ...
    def _initialize_masking_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "fields": {
                ...
                "credit_card": {"masking_type": "full", "pattern": r"\d{4}-\d{4}-\d{4}-\d{4}"},
    ...
    async def _mask_response_data(self, request: Request, response: Response) -> Response:
        """レスポンスデータをマスキング"""
        try:
            # レスポンスボディを取得
            response_body = b""
    ...
    def _mask_data_recursive(self, data: Any, request: Request) -> Any:
        ...
        try:
            if isinstance(data, dict):
                return {key: self._mask_data_recursive(value, request) for key, value in data.items()}
    ...
    def _mask_string_data(self, data: str, request: Request) -> str:
        ...
        try:
            ...
            # ロール別のマスキング設定を取得
            role_config = self.masking_config["roles"].get(user_role, self.masking_config["default"])
    ...
    def _mask_field_by_name(self, field_name: str, field_value: str, masking_type: str = "partial") -> str:
        """フィールド名に基づいてマスキング"""
        try:
            ...
            if pattern:
                ...
                if field_masking_type == "full":
                    ...
                    return re.sub(pattern, lambda m: m.group(1) + "*" * (len(m.group()) - 2) + m.group(2) if len(m.groups()) >= 2 else "*" * len(m.group()), field_value)
...
class SelectiveDataMaskingMiddleware(DataMaskingMiddleware):
    ...
    def __init__(self, app, mask_fields: Optional[List[str]] = None, exclude_fields: Optional[List[str]] = None):
        super().__init__(app)
    ...
    def _mask_data_recursive(self, data: Any, request: Request, field_name: str = "") -> Any:
            ...
            if isinstance(data, dict):
                ...
                for key, value in data.items():
                    ...
                    if self._should_mask_field(current_field):
                        masked_dict[key] = self._mask_data_recursive(value, request, current_field)
                    else:
                        masked_dict[key] = value
                return masked_dict
            elif isinstance(data, list):
                return [self._mask_data_recursive(item, request, field_name) for item in data]
            elif isinstance(data, str):
