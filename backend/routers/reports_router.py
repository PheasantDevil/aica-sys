...
async def generate_report(
    ...
    try:
        ...
        background_tasks.add_task(
            generate_report_background, report_id, current_user.id, db
        )
...
async def generate_report_content(report_id: str, db: Session) -> dict:
    ...
    try:
        ...
        articles = (
            db.query(Article)
            .filter(Article.created_at >= datetime.now() - timedelta(days=30))
            .limit(50)
            .all()
        )
        ...
        trends = (
            db.query(Trend)
            .filter(Trend.created_at >= datetime.now() - timedelta(days=30))
            .limit(20)
            .all()
        )
        ...
        newsletters = (
            db.query(Newsletter)
            .filter(Newsletter.created_at >= datetime.now() - timedelta(days=30))
            .limit(10)
            .all()
        )
        ...
        if report_id == "typescript-trends-2024":
            content = await generate_typescript_trends_report(
                articles, trends, newsletters, ai_analyzer, content_generator
            )
        elif report_id == "nextjs-performance-guide":
            content = await generate_nextjs_performance_report(
                articles, trends, ai_analyzer, content_generator
            )
        elif report_id == "ai-development-tools":
            content = await generate_ai_tools_report(
                articles, trends, ai_analyzer, content_generator
            )
        else:
...
async def generate_typescript_trends_report(
    articles, trends, newsletters, ai_analyzer, content_generator
):
...
async def generate_nextjs_performance_report(
    articles, trends, ai_analyzer, content_generator
):
