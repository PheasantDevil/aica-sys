...
async def get_audit_events(
    ...
    """
    try:
        logger.info(f"API: Fetching audit events with filters: type={event_type}, user_id={user_id}")
...
async def get_resource_audit_events(
    ...
    """
    try:
        logger.info(f"API: Fetching audit events for resource: {resource_type}/{resource_id}")
        ...
        logger.error(f"Error fetching audit events for resource {resource_type}/{resource_id}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_resource_activity_stats(
    start_date: Optional[datetime] = Query(None, description="Start date for statistics"),
    end_date: Optional[datetime] = Query(None, description="End date for statistics"),
    limit: int = Query(10, ge=1, le=100, description="Maximum number of resources to return"),
    db: Session = Depends(get_db),
