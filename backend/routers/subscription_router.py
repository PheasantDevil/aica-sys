...
async def get_subscription_plans(db: Session = Depends(get_db)):
    ...
    try:
        plans = (
            db.query(SubscriptionPlan).filter(SubscriptionPlan.is_active == True).all()
        )
        return {
            "plans": [
                {
...
async def handle_subscription_updated(subscription_data, db: Session):
    ...
    try:
        ...
        if subscription:
            ...
            subscription.cancel_at_period_end = subscription_data[
    ...
    try:
        if invoice_data["subscription"]:
            ...
            if subscription:
                ...
                db.commit()
                logger.info(
                    f"Payment succeeded for subscription: {invoice_data['subscription']}"
                )
    except Exception as e:
    ...
    try:
        if invoice_data["subscription"]:
            ...
            if subscription:
                ...
                db.commit()
                logger.info(
                    f"Payment failed for subscription: {invoice_data['subscription']}"
                )
    except Exception as e:
