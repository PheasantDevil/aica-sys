...
@router.post("/start")
async def start_collection(source: Optional[str] = None, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {"message": "Collection started successfully", "results": results}
...
async def get_collection_stats(db: Session = Depends(get_db)):
    ...
    try:
        ...
        completed_jobs = (
            db.query(CollectionJob).filter(CollectionJob.status == "completed").count()
        )
        failed_jobs = (
            db.query(CollectionJob).filter(CollectionJob.status == "failed").count()
        )
        ...
        recent_jobs = (
            ...
            .filter(CollectionJob.created_at >= yesterday)
            .count()
        )

        recent_analyzed = (
            db.query(AnalysisResult)
            .filter(AnalysisResult.created_at >= yesterday)
            .count()
        )
