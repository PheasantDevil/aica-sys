...
from utils.response_optimizer import (create_conditional_response,
                                      create_error_response, create_etag,
...
async def batch_get_articles(
    ...
    try:
        if len(article_ids) > 50:  # バッチサイズ制限
            raise HTTPException(
                status_code=400,
                detail="Too many articles requested (max 50)"
            )
        ...
        return create_success_response(
            data=results,
            message=f"Batch articles retrieved ({len(results)} items)"
        )
