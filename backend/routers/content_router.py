...
async def get_articles(
    ...
    status: Optional[str] = Query(
        None, description="記事ステータス（published, draft, archived）"
    ),
    ...
    articles = (
        query.order_by(AutomatedContentDB.created_at.desc())
        .offset(offset)
        .limit(page_size)
        .all()
    )
    ...
    for article in articles:
        article_responses.append(
            ArticleResponse(
                ...
                tags=(
                    article.content_metadata.get("tags", [])
                    if article.content_metadata
                    else []
                ),
                quality_score=article.quality_score,
                status=article.status,
                read_time=(
                    article.content_metadata.get("read_time", 5)
                    if article.content_metadata
                    else 5
                ),
    ...
    return ArticleListResponse(
        articles=article_responses, total=total, page=page, page_size=page_size
    )
...
async def get_article(article_id: int, db: Session = Depends(get_db)):
    ...
    return ArticleResponse(
        ...
        content=article.content or "",
        tags=(
            article.content_metadata.get("tags", []) if article.content_metadata else []
        ),
        quality_score=article.quality_score,
        status=article.status,
        read_time=(
