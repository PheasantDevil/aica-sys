...
class MonitoringService:
    ...
    async def _check_external_services(self) -> HealthCheck:
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        ...
                        if response.status in [200, 401, 403]:  # 401/403も正常（認証エラーは正常）
                            healthy_services += 1
            except Exception:
                pass
        ...
        return HealthCheck(
            ...
            message=message,
            timestamp=datetime.utcnow(),
            details={"healthy_services": healthy_services, "total_services": total_services},
        )
    ...
    async def _check_system_resources(self) -> HealthCheck:
        ...
        elif cpu_percent > self.thresholds["cpu_usage"] * 0.8 or \
             memory.percent > self.thresholds["memory_usage"] * 0.8 or \
             disk.percent > self.thresholds["disk_usage"] * 0.8:
            status = HealthStatus.WARNING
    ...
    async def _evaluate_alert_conditions(self, metric: Metric):
        ...
        alert_conditions = {
            ...
            "memory_usage_percent": (self.thresholds["memory_usage"], AlertLevel.CRITICAL),
    ...
    async def _create_alert(self, level: AlertLevel, title: str, message: str, service: str):
        """アラートを作成"""
