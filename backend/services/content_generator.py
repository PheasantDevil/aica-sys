...
class ContentGenerator:
    ...
    async def generate_content(
        ...
        prompt = self._generate_prompt(
            content_type, relevant_results, target_audience, tone
        )
    ...
    def _filter_relevant_results(
        self, results: List[AnalysisResult], content_type: ContentType
    ) -> List[AnalysisResult]:
        """関連する分析結果をフィルタリング"""
        # 重要度とトレンドスコアでソート
        sorted_results = sorted(
            results,
            key=lambda x: (x.importance_score + x.trend_score) / 2,
            reverse=True,
        )
...
常に高品質で実用的なコンテンツを生成してください。"""
    ...
    def _parse_generated_content(
        ...
        return {"title": title, "summary": summary, "content": content, "tags": tags}
    ...
    def _generate_metadata(
        ...
        return {
            ...
            "avg_importance_score": sum(r.importance_score for r in results)
            / len(results),
    ...
    async def generate_image(
        self, prompt: str, style: str = "technical"
    ) -> Optional[str]:
        """画像を生成"""
    ...
    async def generate_article(
        self, analysis_results: List[AnalysisResult], topic: str = None
    ) -> GeneratedContent:
        """記事を生成"""
        # 特定のトピックに絞り込む場合
        if topic:
            filtered_results = [
                r for r in analysis_results if topic.lower() in r.key_topics
            ]
    ...
    async def generate_social_posts(
        self, analysis_results: List[AnalysisResult], count: int = 3
        ...
        high_importance = sorted(
            analysis_results, key=lambda x: x.importance_score, reverse=True
        )[:count]
    ...
    async def generate_technical_guide(
        self, analysis_results: List[AnalysisResult], topic: str
    ) -> GeneratedContent:
        """技術ガイドを生成"""
        ...
        relevant_results = [
            ...
            if topic.lower() in r.key_topics or topic.lower() in r.category
        ]
