...
class DataClassificationService:
            ...
            for rule_name, rule in self.classification_rules.items():
                ...
                for match in matches:
                    classification = {
                        ...
                        "match": match.group(),
                        "position": match.span(),
                        "confidence": self._calculate_confidence(match.group(), rule_name)
                    }
                    ...
                    # 最高機密度を更新
                    if self._is_higher_sensitivity(rule["sensitivity"].value, classification_result["highest_sensitivity"]):
                        classification_result["highest_sensitivity"] = rule["sensitivity"].value

            # 保持期間を計算
            classification_result["retention_days"] = self._calculate_retention_period(classification_result["categories"])
            
            # マスキング要件を判定
            classification_result["masking_required"] = self._requires_masking(classification_result["categories"])
            
            # セットをリストに変換
            classification_result["categories"] = list(classification_result["categories"])
            
            logger.info(f"Data classified: {len(classification_result['classifications'])} matches found")
    ...
    def _is_higher_sensitivity(self, sensitivity1: str, sensitivity2: str) -> bool:
        ...
        return sensitivity_levels.get(sensitivity1, 0) > sensitivity_levels.get(sensitivity2, 0)
    ...
    def mask_data(self, data: str, masking_type: str = "partial") -> str:
        ...
        try:
            ...
            # 各マッチをマスキング
            for classification in reversed(classification_result["classifications"]):  # 逆順で処理
                ...
                if masking_type == "full":
                    ...
                    if len(match_text) <= 4:
                        ...
                        masked_text = match_text[:2] + "*" * (len(match_text) - 4) + match_text[-2:]
                else:
    ...
    def get_data_label(self, classification_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            label = {
                ...
                "retention_until": (datetime.utcnow() + timedelta(days=classification_result["retention_days"])).isoformat(),
    ...
    def should_retain_data(self, classification_result: Dict[str, Any], created_at: datetime) -> bool:
        """
        データを保持すべきかどうかを判定
    ...
    def get_expired_data_candidates(self, data_records: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        保持期間が過ぎたデータの候補を取得
    ...
    def update_classification_rules(self, new_rules: Dict[str, Dict]) -> None:
        ...
        try:
            ...
            logger.info(f"Updated classification rules: {len(new_rules)} rules added/modified")
            
        except Exception as e:
            logger.error(f"Error updating classification rules: {e}")
            raise

    def get_classification_summary(self, data_records: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        データ分類のサマリーを取得
        ...
        try:
            ...
            for record in data_records:
                if "classification_result" in record:
                    ...
                    summary["sensitivity_distribution"][sensitivity] = summary["sensitivity_distribution"].get(sensitivity, 0) + 1
                    ...
                    summary["retention_summary"][retention_range] = summary["retention_summary"].get(retention_range, 0) + 1
