...
class BackupService:
    ...
    def _initialize_backup_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "files": {
                "enabled": True,
                "directories": ["/app/uploads", "/app/logs", "/app/config"],
    ...
    async def _backup_databases(
        self, backup_dir: str, backup_record: Dict[str, Any]
    ) -> None:
        """データベースをバックアップ"""
        try:
    ...
    async def _backup_postgresql(
        self, databases_dir: str, backup_record: Dict[str, Any]
    ) -> None:
        """PostgreSQLをバックアップ"""
        try:
    ...
    async def _copy_directory(
        self, src: str, dst: str, backup_record: Dict[str, Any]
    ) -> None:
        """ディレクトリをコピー"""
        try:
    ...
    async def _upload_to_s3(
        self, backup_file: str, backup_record: Dict[str, Any]
    ) -> None:
        """S3にアップロード"""
        try:
            ...
            key = (
                f"backups/{backup_record['backup_id']}/{os.path.basename(backup_file)}"
            )
    ...
    async def restore_backup(
        self, backup_id: str, storage_provider: StorageProvider = StorageProvider.LOCAL
    ) -> Dict[str, Any]:
        """
        バックアップから復元
    ...
    async def _download_backup(
        self, backup_id: str, storage_provider: StorageProvider
    ) -> str:
        """バックアップをダウンロード"""
        try:
    ...
    def get_backup_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            failed_backups = len(
                [b for b in self.backup_history if b["status"] == "failed"]
            )
