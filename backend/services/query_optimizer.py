...
class QueryOptimizer:
    ...
    async def get_articles_optimized(
        ...
        try:
            ...
            query = select(Article).options(
                selectinload(Article.tags), selectinload(Article.author)
            )
    ...
    async def get_trends_optimized(
        ...
        """トレンドを最適化されたクエリで取得"""
        try:
            query = select(Trend).options(selectinload(Trend.tags))
            ...
            trends_result, count_result = await asyncio.gather(trends_task, count_task)
    ...
    async def get_newsletters_optimized(
        ...
        try:
            query = select(Newsletter).options(selectinload(Newsletter.tags))
            ...
            return {
                "newsletters": [
                    self._serialize_newsletter(newsletter) for newsletter in newsletters
                ],
                "total": total,
                "skip": skip,
                "limit": limit,
    ...
    async def _get_content_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            newsletters_count_task = self.session.execute(
    ...
    def _serialize_newsletter(self, newsletter: Newsletter) -> Dict[str, Any]:
        ...
        return {
            ...
            "updated_at": (
                newsletter.updated_at.isoformat() if newsletter.updated_at else None
            ),
            "status": newsletter.status,
            "scheduled_at": (
                newsletter.scheduled_at.isoformat() if newsletter.scheduled_at else None
            ),
        }
