...
class AnalysisAgent:
    ...
    async def _analyze_single_item(self, data: Dict) -> Optional[AnalysisResult]:
        ...
        try:
            ...
            result = AnalysisResult(
                ...
                title=data["content"][:100] + "..." if len(data["content"]) > 100 else data["content"],
            ...
            logger.error(f"Failed to analyze item {data.get('source_id', 'unknown')}: {e}")
            return None
    ...
    async def get_analysis_summary(self) -> Dict:
        ...
        try:
            ...
            recent_analyzed = self.db.query(AnalysisResult).filter(
                AnalysisResult.created_at >= datetime.utcnow().replace(hour=0, minute=0, second=0, microsecond=0)
            ).count()
            
            # Get sentiment distribution
            positive_count = self.db.query(AnalysisResult).filter(
                AnalysisResult.sentiment == Sentiment.POSITIVE
            ).count()
            neutral_count = self.db.query(AnalysisResult).filter(
                AnalysisResult.sentiment == Sentiment.NEUTRAL
            ).count()
            negative_count = self.db.query(AnalysisResult).filter(
