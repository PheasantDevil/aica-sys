...
| `GROQ_API_KEY`           | （GitHub Secrets から取得）                                                                                          | ⚠️ 要設定                  |
| `STRIPE_SECRET_KEY`      | （Stripe 本番キー）                                                                                                  | ⚠️ 要設定（次のステップ）  |
| `STRIPE_PUBLISHABLE_KEY` | （Stripe 本番キー）                                                                                                  | ⚠️ 要設定（次のステップ）  |
...
- [ ] 環境変数設定（Supabase 関連）
- [ ] 動作確認（URL アクセス、ヘルスチェック）

### Render

- [ ] デプロイ状態確認（Status: Live）
- [ ] 環境変数設定（DATABASE_URL, GROQ_API_KEY 等）
- [ ] 動作確認（URL アクセス、ヘルスチェック）
