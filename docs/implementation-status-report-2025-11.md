...
**2.5 投稿分析とレポート** ⏱️ 0.5 日 ✅ _完了_

- [x] 投稿成功/失敗ログ記録（`social_post_logs` テーブル + Analytics イベント化）
- [x] エンゲージメント追跡準備（Tweet public metrics を自動取得する `refresh_post_metrics` 実装）
- [x] 週次レポート作成（`scripts/generate_social_post_report.py` でレポート自動保存）
...
**9.2 コミッション計算** ⏱️ 3 日 ✅ _完了_

- [x] コミッション計算ロジック（ティア別・閾値・段階制ルール対応）
- [x] 支払い管理（残高閾値ベースの自動支払いリクエスト生成）
- [x] レポート生成（期間指定のコミッションレポート + `scripts/generate_affiliate_commission_report.py`）
...
| Day 5 PM | ✅ OGP 画像作成・最適化（/api/og + プレビュー CLI） | 4 時間   | P1     |
