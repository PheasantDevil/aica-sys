...
async function setupStripeProducts() {
  ...
  try {
    ...
    const basicProduct = await stripe.products.create({
      ...
      description:
        'Monthly subscription for TypeScript ecosystem insights and curated content',
      metadata: {
        type: 'subscription',
        tier: 'basic',
      },
    ...
    const premiumProduct = await stripe.products.create({
      ...
      description:
        'Premium monthly subscription with advanced features and custom reports',
      metadata: {
        type: 'subscription',
        tier: 'premium',
      },
    ...
    log(
      `✅ Premium subscription product created: ${premiumProduct.id}`,
      'green'
    );
    ...
    const reportProduct = await stripe.products.create({
      ...
      description:
        'In-depth analysis report on TypeScript ecosystem trends and insights',
      metadata: {
        type: 'one_time',
        category: 'report',
      },
...
async function main() {
  ...
  try {
    ...
    log(
      '1. Update your environment variables with the webhook secret',
      'yellow'
    );
